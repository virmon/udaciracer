...
async function handleCreateRace() {
	// render starting UI
	renderAt('#race', renderRaceStartView(store.tracks.find(track => parseInt(track.id) === parseInt(store.track_id))))
...
async function getRace(id) {
	...
	try {
		return await fetch(`${SERVER}/api/races/${newId}`, {
			method: 'GET',
			...defaultFetchOpts()
		}).then(res => res.json())
	} catch (err) {
...
async function startRace(id) {
	...
	const newId = parseInt(id) - 1
	try {
		return await fetch(`${SERVER}/api/races/${newId}/start`, {
			method: 'POST',
			...defaultFetchOpts()
		})
	} catch (err) {
...
async function accelerate(id) {
	...
	const newId = parseInt(id) - 1
	try {
		return await fetch(`${SERVER}/api/races/${newId}/accelerate`, {
			method: 'POST',
			...defaultFetchOpts()
		})
	} catch (err) {
